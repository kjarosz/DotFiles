-- Check out https://github.com/lewis6991/gitsigns.nvim?tab=readme-ov-file
require('gitsigns').setup { 
  on_attach = function(bufnr)
    ...
    end, { desc = "Navigate to next [c]hange" })
    ...
    end, { desc = "Navigate to previous [c]hange" })
    ...
    map('n', '<leader>hR', gitsigns.reset_buffer, { desc = "[R]eset buffer staging" })
    map('n', '<leader>hp', gitsigns.preview_hunk, { desc = "[P]review hunk" })
    map('n', '<leader>hb', function() gitsigns.blame_line{full=true} end, { desc = "[B]lame line" })
    map('n', '<leader>tb', gitsigns.toggle_current_line_blame, { desc = "[T]oggle [B]lame for current line" })
